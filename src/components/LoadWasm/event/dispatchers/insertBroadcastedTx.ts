...
export function insertBroadcastedTx(tx: Transaction): Promise<void> {
      ...
      callback: (worker: Worker): Promise<void> =>
        new Promise((_resolve) => {
          const handleEvent = (event: MessageEvent<Message<unknown>>) => {
            if (event.data.type === MessageTypes.TX_INSERTED) {
          ...
          worker.postMessage(new Message(MessageTypes.INSERT_TX, tx));
        }),
    });
